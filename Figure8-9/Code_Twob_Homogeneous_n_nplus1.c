...
{           
    ...
    int n_vec[6]= {2, 3, 5, 9, 99, 999};

    FILE *Optimal_V, *Loss_b1b2, *Loss_b1b2d1;
    Optimal_V = fopen("Twob_n_optd.m", "w");
    Loss_b1b2 = fopen("Twob_n_loss_oneb.m", "w");
    Loss_b1b2d1 = fopen("Twob_n_loss_twob.m", "w");

    fprintf(Optimal_V, "\n\n %%Value* b1 b2 d1 C p\n");
    fprintf(Loss_b1b2, "%%Value Value* b1 b1* b2 b2* d1 d1* C p\n");
    fprintf(Loss_b1b2d1, "%%Value Value* b1 b1* b2 b2* d1 d1* C p\n");

    for (int n_index = 0; n_index < 6; ++n_index)
    {
        n=n_vec[n_index];
        p=(double)n/((double)n+1.);
        ...
        for (int capacity = 0; capacity < 9; ++capacity)
        {
            ...
            //Comparing value with what happens when b1=2, b2=1, d1=1;
...
double* diffusion(double *J, double *Q, double *q, int layer, int depth, int branch, int n, int length)
    {
        ...
        Q[posQ]=1./((double)(n+1))*q[depth-layer+1]*(J[posQ+1]);

        for (i = depth-layer+1; i < depth; ++i)
        {
            Q[i]=(1-q[depth-layer+1])*J[i]+1./((double)(n+1))*q[depth-layer+1]*(J[i+1]);
        }
        posQ=depth;
        ...
        for (i = 1; i < layer; ++i)
        {
            ...
            {
                posQ+=1;
                Q[posQ]=(1-q[depth-layer+1])*J[posQ]+1./((double)(n+1))*(double)n*q[depth-layer+1]*J[posQ-depth+(i-1)]+1./((double)(n+1))*q[depth-layer+1]*J[posQ+1];
            }
        }

            posQ+=depth-layer+1;
            Q[posQ]=1./((double)(n+1))*q[depth-layer+1]*((double)n*J[posQ-(depth-layer+1)]);
...
double reward(int depth, int d1, int b1, int b2, int C, int n)
    {
        ...
        Q[2*depth]= ((double)n/((double)(n+1)))*q[depth]; //Q_1=1  
        Q[depth]= 1-q[depth];     //Q_1=0
        Q[depth-1]= (1./((double)(n+1)))*q[depth]; //Q_1=-1/n
