...
X_01 = load('Value_b_gamma_p_01.m');
...
for i=1:numel(C_vec)
    for j=1:numel(gamma_vec)
        ...
        max=0;
        for l=1:numel(b_vec)
            if X_01(pos,1)>max_value
                max_value=X_01(pos,1);
                max=X_01(pos,2);
            end
        pos=pos+1;
        end
...
text(0.05,0.97,'$p=0.01$','Units','normalized','Color',[0 0 0],'FontSize',14,...
